...
function convertCurrency(amount) {
    var currencyType = $('#sessionCurrency').val();
    if (currencyType == 'gbp') {
        //var convertAmount = (parseFloat(amount * 0.8942).toFixed(2));
        return "&#163;" + amount;
    ...
    mapSVG = function (elem, options) {
        ...
        this.methods = {
            ...
            setLoadingText: function (val) {
                _data.options.loadingText = val
            },
            setLockAspectRatio: function (val) {
                _data.options.lockAspectRatio = MapSVG.parseBoolean(val);
            },
            setOnClick: function (h) {
                _data.options.onClick = h || undefined;
            },
            setMouseOver: function (h) {
                _data.options.mouseOver = h || undefined;
            },
            setMouseOut: function (h) {
                _data.options.mouseOut = h || undefined;
            },
            setBeforeLoad: function (h) {
                _data.options.beforeLoad = h || undefined;
            },
            setAfterLoad: function (h) {
                _data.options.afterLoad = h || undefined;
            },
            setPopoverShown: function (h) {
                _data.options.popoverShown = h || undefined;
            },
            ...
            updateGaugeMinMax: function () {
                ...
                var values = [];
                _data.regions.forEach(function (r) {
                    if (r.gaugeValue != null && r.gaugeValue != undefined)
                        values.push(r.gaugeValue);
                });
                if (values.length > 0) {
...
//                        window.location.href = region.href;
            ...
            showTooltip: function (region) {
              //  $('.seat_select_items').mouseout();
				...
				$('#' + tip).css('cursor', 'pointer');
                    /*$('#' + tip).click(function () {
                        $("#rowid-" + tip).show().siblings(".rowsection").hide();
					...
					$('#noofticket').val('0');
						...
						if(full_block_data){
							...
							$.each(stadium_with_cat_name,function(indx, itm){
								if(parseInt(itm[0])==parseInt(ths_cat)){
									$("#allticktype").val(indx);
									ths_cat2=indx;
								}
							});
							
							$.each(full_block_data, function (indx, itm) {
								if($.inArray(tip, itm) !== -1){
									sn_exst++;
									//break;
								}
							});
							if(sn_exst>0){
								...
								setTimeout(function(){
								 $("[data-blockid='"+tip+"']").show().siblings('.seat_select_items').not($("[data-blockid='"+tip+"']")).hide();
									//$("#seatid-" + tip).show().siblings(".seat_select_items").hide();
									$(".mapsvg-region").css({fill: 'rgb(221, 221, 221)'});
									$(".mapsvg-region").each(function () {
										$(this).css('opacity', '0.1');
										if (tip) {
											$('#' + tip).css('opacity', '1');
											$('#' + tip).css({fill: stadium_block_details[tip]});
										}
									});
								},200);
							}
							else{
								var selectCategory = ths_cat2.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
								$(".mapsvg-region").css({fill: 'rgb(221, 221, 221)'});
								$("#allticktype").trigger("change");
								$.each(stadium_cat_details,function(indx, itm){
									if(parseInt(indx)==parseInt(ths_cat)){
                    ...
                    //$('#seatid-' + tip).mouseover();
                    console.log('ticket_price_info', ticket_price_info)
					if($("#"+tip).data("cat") in ticket_price_info){
						var splitTip = tip.split("-");
						var blockIdLength=0;
						for(var i=0;i<splitTip.length;i++){
							blockIdLength =splitTip.length;
						}	
						if(blockIdLength ==1){
							blockId =tip;
						}else{
						...
						if(tip in ticket_price_info[$("#"+tip).data("cat")]){
							if(parseInt(ticket_price_info[$("#"+tip).data("cat")][tip]['no_of_ticket'])>0){
							    var totalTicketvalue = 0;
								for (var i = 0; i < ticket_price_info[$("#"+tip).data("cat")][tip]['no_of_ticket'].length; i++) {
									  totalTicketvalue += ticket_price_info[$("#"+tip).data("cat")][tip]['no_of_ticket'][i] << 0;
								  }
								_data.tooltip.container.html("Block "+ blockId + '<br/> ' + totalTicketvalue + ' tickets from ' + convertCurrency(Math.min.apply(Math,ticket_price_info[$("#"+tip).data("cat")][tip]['price']))+"<br><br><strong>"+ticket_price_info_with_cat[$("#"+tip).data("cat")]['name']+":</strong><br> Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from "+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']));
							}
							else{
								_data.tooltip.container.html("<strong>"+ticket_price_info_with_cat[$("#"+tip).data("cat")]['name']+":</strong><br> Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from "+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']));
							}
						}
						else{
							_data.tooltip.container.html("<strong>"+ticket_price_info_with_cat[$("#"+tip).data("cat")]['name']+":</strong><br> Total tickets "+ticket_price_info_with_cat[$("#"+tip).data("cat")]['no_of_ticket']+" from "+convertCurrency(ticket_price_info_with_cat[$("#"+tip).data("cat")]['price']));
						...
						_data.tooltip.container.html('<span>Tickets Not Available<span>');
					}
					/*_data.tooltip.container.html('loading...');
					$.ajax({
                        type: "POST",
                        url: base_url + '/admin/game/getSellerDetailsBydata',
                        data: {'blockid': tip, 'match_id': $("#main_matcheventid").val(), cat_id: $("#"+tip).data("cat")},
                        success: function (response)
                        {
                         ...
                         // console.log(response);
							...
							var org_ticket = jsonObject['no_of_Tickets'];
							var org_price = jsonObject['price'];
                            var overall = jsonObject['overAllPrice'];
							var total_category_tickets =jsonObject['total_category_tickets'];
							var total_category_price_from =jsonObject['total_category_price_from'];
                            var object = getJsonArray[0];
                            if (getJsonArray != '') {
                                var category = category;
                                var price = object['price'];
                                var noTickets = object['quantity'] - object['sold'];
                                var priceType = object['price_type'];
                                if (priceType == 'pound') {
                                    var currencySym = '£';
                                }
                                if (priceType == 'dollar') {
                                ...
                                if (status == 'OK') {
                                    _data.tooltip.container.html("Block "+ object['ticket_block'] + '<br/> ' + org_ticket + ' tickets from ' + convertCurrency(org_price)+"<br><br><strong>"+category+":</strong><br> Total tickets "+total_category_tickets+" from "+convertCurrency(total_category_price_from));
                                }
                            } else {
								
                                if(parseInt(total_category_tickets) > 0){
									_data.tooltip.container.html("<strong>"+category+":</strong><br> Total tickets "+total_category_tickets+" from "+convertCurrency(total_category_price_from));
								}
								else{
									_data.tooltip.container.html('<span style="color:#F00;">Tickets Not Available.<span>');
								}
            ...
            popoverAdjustPosition: function () {
                if (!_data.$popover || !_data.$popover.data('point'))
                    return;
            ...
            mouseOverHandler: function (e) {
				...
				//Custom code by senthil
				if(current_category==0){
					var catt_id=$("#"+e.currentTarget.id).data("cat");
					$(".mapsvg-region").css({fill: 'rgb(221, 221, 221)'});
					$.each(stadium_cat_details,function(indx, itm){
						if(parseInt(indx)==parseInt(catt_id)){
							$.each(itm,function(indx2, itm2){
								$('#'+itm2).css({fill: stadium_block_details[itm2]});
            ...
            init: function (opts, elem) {
               ...
               // alert(map_url + _data.options.source + '?v=' + _data.options.svgFileVersion);
                // GET the map by ajax request
               
                $.ajax({url: map_url + _data.options.source + '?v=' + _data.options.svgFileVersion})
