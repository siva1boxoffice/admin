...
//            $('#noBlocks').val(noBlocks.join(','));
        ...
        init: function (options) {
            ...
            $.get(base_url+'/admin/game/stadium/check_stadium', {action: 'check_php'}).done(function (resp) {
                if (resp == "1")
                    _this.setPHPYes();
                else
                    _this.setPHPNo();
                ...
                if (onEditMapScreen) {
                    ...
                    _data.options.mapsvg_options.afterLoad = function () {
                        ...
                        $('#tab_regions').on('change', '.mapsvg-region-link', function (e) {
                            ...
                            let block_id = $(e.target).closest('.mapsvg-region-row').data('region-id');
                            let fillColor = hexToRgbA($('input[name="regions[' + block_id + '][fill]"]').val());
                            let category = $(this).val();

                            let blockIdSelector = $('#stadiumForm').find('[name=blockid]').val();
                            if(blockIdSelector != undefined && blockIdSelector != "") {
                                blockIdSelector = blockIdSelector.split(',');
                            } else {
                                blockIdSelector = [];
                            }
                            blockIdSelector.push(block_id);
                            $('#stadiumForm').find('[name=blockid]').val(blockIdSelector.join(","));

                            let fillColorSelector = $('#stadiumForm').find('[name=fillcolor]').val();
                            console.log('fillColorSelector', fillColorSelector)
                            if(fillColorSelector != undefined && fillColorSelector != "") {
                                fillColorSelector = fillColorSelector.split('-');
                            } else {
                                fillColorSelector = [];
                            }
                            fillColorSelector.push(fillColor);
                            $('#stadiumForm').find('[name=fillcolor]').val(fillColorSelector.join("-"));

                            let categoryNameSelector = $('#stadiumForm').find('[name=categoryname]').val();
                            if(categoryNameSelector != undefined && categoryNameSelector != "") {
                                categoryNameSelector = categoryNameSelector.split(',');
                            } else {
                                categoryNameSelector = [];
                            }
                            categoryNameSelector.push(category);
                            $('#stadiumForm').find('[name=categoryname]').val(categoryNameSelector.join(","));
                        ...
                        $('#tab_regions').on('keyup change', '.mapsvg-region-color', function (e) {
                            $('.preloader_full').css('display', 'none');
