           ...
           // $('#mapsvg-save')._button('loading');
            ...
            for (var key in options) {
                if (options.hasOwnProperty(key)) {
                    //console.log(options.regions);
                    $.each(options.regions, function (indx, itm) {

                        if (!itm.href) {
                            //	console.log(itm);
                            delete options.regions[indx];
                        }
                    });
                }
            }
            var getRecord = options.regions;
...
//            var noBlocks = [];
            var noTickets = [];
            var categoryName = [];
            for (var key in getRecord) {
                if (getRecord.hasOwnProperty(key)) {
                    // noTickets.push(getRecord[key]["tooltip"]);
//                    noTickets.push(getRecord[key]["data"]);
//                     noBlocks.push(getRecord[key]["data"]);
                    blockId.push(getRecord[key]["id"]);
                    fillColor.push(getRecord[key]["fill"]);
                    categoryName.push(getRecord[key]["href"]);
                }
            }
            $('#splitCode').val(getRecord);
            $('#stadiumViewName').val($('#stadium_name').val());
            $('#stadiumViewName_ar').val($('#stadium_name_ar').val());
            $('#stadiumName').val(options.title);
            $('#stadiumWidth').val(options.width);
            $('#stadiumHeight').val(options.height);
            $('#stadiumImagePath').val(options.source);
            $('#blockId').val(blockId.join(','));
            $('#inpt_gamecategory').val($('#gamecategory').val());
            $('#inpt_country').val($('#stadium_country').val());
            $('#inpt_city').val($('#stadium_city').val());
//            $('#price').val(price.join(','));
            // $('#noTickets').val(noTickets.join(','));
//            $('#noBlocks').val(noBlocks.join(','));
            // $('#fillColor').val(fillColor.join('-'));
            $('#categoryName').val(categoryName.join(','));
            ...
            // if (existing_json_data.regions) {
            //     $.each(existing_json_data.regions, function (n_itm, nw_jsn) {
            //         var temp = nw_jsn;
            //         var id_var = [];

            //         new_json_data[n_itm] = temp;
            //     });
            // }
            if (options.regions) {
                $.each(options.regions, function (itm, opt_vlu) {

                    var temp = opt_vlu;
                    new_json_data[itm] = temp;

                });
            }
            delete options['regions'];
            options.regions = new_json_data;
            ...
            $('#stadiumCode').val(JSON.stringify(options));
            ...
            var Title = _this.convertToText(options.title);
        ...
        init: function (options) {
            ...
            $.get(base_url + '/game/stadium/check_stadium', {action: 'check_php'}).done(function (resp) {
                if (resp == "1")
                    _this.setPHPYes();
                else
                    _this.setPHPNo();
            ...
            $(document).ready(function () {
                ...
                if (onEditMapScreen) {
                    ...
                    _data.options.mapsvg_options.afterLoad = function () {
                        ...
                        var updated_block_id = [];

                $('#tab_regions').on('change', '.mapsvg-region-link', function (e) {
                ...
                // var dataname = $("#default_category").find(':selected').data('name');
                    ...
                    var rid = $(this).parents("tr").attr('data-region-id');
                    ...
                    if (stadiumId != '') {
                        ...
                        var color_id = datacolor;
                         ...
                         $('#picker_' + rid).css("background",datacolor);
                         $("#"+rid).css("fill" , datacolor);
                        ...
                        $.ajax({
                            type: 'POST',
                            url: base_url + 'game/update_statdium_block',
                    ...
                    return;
                        ...
                        $('#tab_regions').on('click', '.mapsvg-region-color', function (e) {
                            $('.preloader_full').css('display', 'none');
                            // var rid = $(e.target).closest('.mapsvg-region-row').data('region-id');
                            ...
                            if (stadiumId != '') {
                                $.ajax({
                                    type: 'POST',
                                    url: base_url + 'game/update_statdium_block',
                                    data: {'stadiumid': stadiumId, 'block_id': rid, 'color': hexToRgbA($('input[name="regions[' + rid + '][fill]"]').val()), 'href': $('select[name="regions[' + rid + '][href]"]').val()},
                                    success: function (response) {
                        ...
                        if (stadiumId != '') {
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    var directData = jsonObject['directData'];
                                    var object = getJsonArray[0];
                                    var mapcode = $.parseJSON(object['map_code']);
                                   // console.log(mapcode);
                                   // glob_mapcode = mapcode.regions;
                                    ...
                                    $.each(directData, function (key, value) {
                                        ...
                                        //if (updated_block_id.indexOf(key) == -1) {
                                        //if ($('input[name="regions[' + key + '][fill]"]').length == 1) {
                                          if(value.id){

                                        $('#mapsvg-region-' + key).attr("data-category",value.href);
                                        $('#mapsvg-region-' + key).addClass("cc-"+value.href);

                                        $('#category_name_' + key).html(value.seat_category);



                                       // $('#picker_' + key).css('background-color', value.fill);
                                        $('input[name="regions[' + key + '][fill]"]').val( rgb2hex(value.fill));
                                        $('select[name="regions[' + key + '][href]"]').val(value.href);


                                        $('input[name="regions[' + key + '][tooltip]"]').val(value.href);



                                        $('input[name="regions[' + key + '][data]"]').val(value.data);
                                        updated_block_id.push(key);
                                         if(value.fill !=""){
                                            //console.log(key);
                                          //  msvg.getRegion(key).setFill(value.fill);
                                        }
                                    }
                                        //}
                                        //	}
                        ...
                        $(document).on('click', '.mapsvg-region-row', function (e) {

                            if($(this).find('.mapsvg-region-fields').is(':visible')) {return;}
                            var stadiumId = $('#hiddenStadiumId').val();
                            var getUrl = window.location;
                            // var base_url = getUrl.origin + '/' + getUrl.pathname.split('/')[1]; // + '/' + getUrl.pathname.split('/')[1]
                            var glob_mapcode;
                                    ...
                                    success: function (response) {
                                        ...
                                        $.each(mapcode.regions, function (key, value) {
                                           ...
                                           if(value.id !=""){
                                                ...
                                                $('#mapsvg-region-' + key).attr("data-category","1");
                                                $('#mapsvg-region-' + key).attr("data-href",value.href);

                                                $('#picker_' + key).css('background-color', value.fill);
                                                $('input[name="regions[' + key + '][fill]"]').val(rgb2hex(value.fill));
                                                $('select[name="regions[' + key + '][href]"]').val(value.href);
                                                $('input[name="regions[' + key + '][tooltip]"]').val(value.tooltip);
                                                $('input[name="regions[' + key + '][data]"]').val(value.data);
                                                updated_block_id.push(key);
                                                 if(value.fill !=""){
                                               // msvg.getRegion(key).setFill(value.fill);
                            ...
                            if (!$(this).hasClass('active')) {
                                ...
                                _data.tableRegionsActiveRow.find('.cpicker').colorpicker().on('changeColor.colorpicker', function (event) {
                                   // var rid = $(event.target).closest('.mapsvg-region-row').data('region-id');
                                    ...
                                    msvg.getRegion(rid).setFill(hexToRgbA($('input[name="regions[' + rid + '][fill]"]').val()));
                                });
                                $('.nano').nanoScroller({scrollTo: _data.tableRegionsActiveRow});

                            }
                            $('#mapsvg-mapform-container').trigger('change');
                            $('#mapsvg-mapform-container').trigger('keyup');
                            $('#mapsvg-mapform-container').trigger('paste');
                        });
                        ...
                        $('body').on('click', '.preview_style', function () {
                        ...
                        $('#table-regions .cpicker').colorpicker().on('changeColor.colorpicker', function (event) {
                           ...
                           //console.log(color_id);
                            ...
                            if ($(this).closest('.form-group').find('input').val() == "")
                                color = null;
                            msvg.getRegion(rid).setFill(rgb2hex(color_id));
                            ...
                            if (stadiumId != '') {
                                ...
                                $.ajax({
                                    type: 'POST',
                                    url: base_url + 'game/update_statdium_block',
                                    data: {'stadiumid': stadiumId, 'block_id': rid, 'color': hexToRgbA(color_id) , 'href': $('select[name="regions[' + rid + '][href]"]').val()},
                                    success: function (response) {
                                    }
                                });
                            }
                  ...
                  function updateAll(event) {
                      //  console.log(event);
                        // alert(event.target.value);
                              ...
                              var color_name = event.target.value ?  hexToRgbA(event.target.value) : "";
                        ...
                        $('.cpicker').colorpicker();
                           ...
                           $('.category_picker').colorpicker();

                    $('.category_picker').colorpicker().on('changeColor.colorpicker', function (event) {


                            var input = $(this).find('input');
                            $(this).parents("tr").find(".stadium_color").val(input.val());
                             var old_category  = $(this).parents("tr").attr('data-category');
                              var staduim_id  = $("#hiddenStadiumId").val();
                              var category = $(this).parents("tr").find('.stadium_category_by_color').val();;
                              var color_name = input.val();
                              //  var color_name =  $(this).parents("tr").find('.stadium_color').val();
                              var id  =  $(this).parents("tr").attr('data-id');
                            console.log(color_name);
                            

                             $(".cc-" + category ).find(".cpicker i").css("background-color" , color_name );
