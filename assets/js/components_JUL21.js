...
$(document).ready(function () {
    if (
        ...
        $(".toast-trigger").length &&
            ...
            $("#primary-toast-demo").on("click", function () {
                $("body").hasClass("is-dark") ? notyf.open({ type: "accent", message: "Please be careful when driving back to home." }) : notyf.open({ type: "primary", message: "Please be careful when driving back to home." });
            })),
        ...
        $(".filepond").length && FilePond.create(document.querySelector(".filepond")),
        $(".filepond-2-grid").length && FilePond.create(document.querySelector(".filepond-2-grid")),
        $(".filepond-3-grid").length && FilePond.create(document.querySelector(".filepond-3-grid")),
        ...
        $("#lightgallery").length && lightGallery(document.getElementById("lightgallery")),
        $("#videogallery").length && lightGallery(document.getElementById("videogallery")),
        $("#pickaday-datepicker").length)
    )
        new Pikaday({ field: document.getElementById("pickaday-datepicker"), format: "D MMM YYYY", onSelect: function () {} });
    if (
        (/*$("#bulma-datepicker-1").length && bulmaCalendar.attach("#bulma-datepicker-1", { color: themeColors.primary, lang: "en" }),*/
        $("#bulma-datepicker-2").length && bulmaCalendar.attach("#bulma-datepicker-2", { displayMode: "dialog", startDate: new Date("02/11/2018"), minDate: "01/01/2018", maxDate: "12/31/2018", color: themeColors.primary, lang: "en" }),
        $("#bulma-datepicker-3").length && bulmaCalendar.attach("#bulma-datepicker-3", { displayMode: "inline", startDate: new Date("02/11/2018"), minDate: "01/01/2018", maxDate: "12/31/2018", color: themeColors.primary, lang: "en" }),
        $("#bulma-datepicker-4").length && bulmaCalendar.attach("#bulma-datepicker-4", { color: themeColors.primary, lang: "en" }),
       // $("#bulma-datepicker-5").length && bulmaCalendar.attach("#bulma-datepicker-5", { color: themeColors.primary, lang: "en" }),
        $("#bulma-datepicker-6").length && bulmaCalendar.attach("#bulma-datepicker-6", { color: themeColors.primary, lang: "en" }),
        $("#bulma-datepicker-7").length && bulmaCalendar.attach("#bulma-datepicker-7", { color: themeColors.primary, lang: "en" }),
    ...
    if ($("#autocomplete-demo-simple").length) {
        $("#autocomplete-demo-simple").easyAutocomplete({
            ...
            highlightPhrase: !1,
            list: { maxNumberOfElements: 5, showAnimation: { type: "fade", time: 400, callback: function () {} }, match: { enabled: !0 }, onChooseEvent: function () {} },
        });
    }
    ...
    if ($(".noui-base-slider").length)
        for (var e = document.getElementsByClassName("noui-base-slider"), t = 0; t < e.length; t++) noUiSlider.create(e[t], { start: [10, 90], connect: !0, orientation: "horizontal", range: { min: 0, max: 100 } });
    if ($(".noui-vertical-slider").length) {
        var o = document.getElementsByClassName("noui-vertical-slider");
        for (t = 0; t < o.length; t++) noUiSlider.create(o[t], { start: [10, 90], connect: !0, orientation: "vertical", range: { min: 0, max: 100 } });
    }
    ...
    if (
        ...
        SUNEDITOR.create(document.getElementById("sun-editor") || "sun-editor", { width: "100%", height: 250, placeholder: "Write your text here..." });
    if ($("#sun-editor-balloon").length) SUNEDITOR.create(document.getElementById("sun-editor-balloon") || "sun-editor-balloon", { width: "100%", height: 250, placeholder: "Write your text here...", mode: "balloon-always" });
    ...
    $("#alertify-demo-1").length &&
        $("#alertify-demo-1").on("click", function () {
            initConfirm("Standard Alert", "Are you sure you want to perfom this action? You won't be able to recover or to revert it.", !1, !1, "Delete", "Cancel");
        }),
        $("#alertify-demo-2").length &&
            $("#alertify-demo-2").on("click", function () {
                initConfirm("Custom Alert", "You can pass a callback function as a callback parameter to define what happens after the confirm button is clicked.", !1, !1, "Callback", "Cancel", function (e) {
