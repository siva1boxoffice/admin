        ...
        save: function () {
            ...
            for (var key in getRecord) {
                if (getRecord.hasOwnProperty(key)) {
                    // price.push(getRecord[key]["data"]["price"]);
                     // noTickets.push(getRecord[key]["data"]["no_tickets"]);
                     // noBlocks.push(getRecord[key]["data"]["id_no_spaces"]);
                    blockId.push(getRecord[key]["id"]);
                    fillColor.push(getRecord[key]["fill"]);
                    categoryName.push(getRecord[key]["tooltip"]);
                }
            }
...
//            $('#stadiumForm').submit();
        ...
        init: function (options) {
            ...
            $.get(base_url + '/admin/game/checkStadium', {action: 'check_php'}).done(function (resp) {
                if (resp == "1")
                    _this.setPHPYes();
                else
                    _this.setPHPNo();
