      ...
      <div class="page-title-box">
         <div class="container-fluid">
            <!-- <div class="page-title dflex-between-center">
               <h3 class="mb-1">Other Events Category</h3>
            </div> -->
            <div class="page-title dflex-between-center">
               <h3 class="mb-1">Other Events Category</h3>
               <div class="float-sm-right mt-3 mt-sm-0 add_team_s">
                  <a href="<?php echo base_url(); ?>event/other_events_category/add_category"  class="btn btn-success mb-2">Add New</a>
               </div>
             </div>
         </div>
      </div>
         ...
         <div class="container-fluid">
            ...
            <div class="card">
               <div class="card-body">
                                       ...
                                       <div class="btn-group">
                                          <div class="dropdown">
                                             <button class="btn btn-light dropdown-toggle ticket_search_filter" type="button"
                                                ...
                                                aria-expanded="false">
                                                Category Name <i class="mdi mdi-chevron-down"></i>
                                             </button>
                  ...
                  <div class="">
                     <table style='width:100% !important' id="other-category-list" class="table table-hover table-nowrap mb-0 tournament">
...
<script>
    ...
    var overlay = $('#overlay');
   var Dtable = $('#other-category-list').DataTable({
         'info': false,
         'serverSide': true,
         'serverMethod': 'post',
         'scrollX': !0,
         ...
         "bSort": false,
         "ajax": {
            url: base_url + 'event/get_other_category_list',
            data: function (d) {
	  ...
	  $("body").on("click",".status_reset",function(){
	   ...
	   $("body").on("click",".split_reset",function(){
         ...
         var checkedIds = [];
            ...
            const category_name=document.getElementById('ticket_type').value;         
            const status=checkedIds;

            var filters = {
               other_category_category_name: category_name,
               other_category_status: checkedIds
            // ... Add other filters
            };
            sessionStorage.setItem('other_category', JSON.stringify(filters));
          Dtable.draw();        
         ...
         function resetFilters() {
               // Save the filter values in session storage
                     var filters = {
                        other_category_category_name: "",
                        other_category_status: ""
                     // ... Add other filters
                     };
                     sessionStorage.setItem('other_category', JSON.stringify(filters));
                  Dtable.draw();        
...
var storedFilters = sessionStorage.getItem('other_category');
  if (storedFilters) {
      var filters = JSON.parse(storedFilters);     
     
      var category_name = filters.other_category_category_name;
      var status =filters.other_category_status;
...
$(".check_box input[type='checkbox']").each(function() {
  ...
  var filters = JSON.parse(sessionStorage.getItem('other_category'));

  // Check if sales_summary_seller_name has a value
  if (filters["other_category" + argName] && filters["other_category" + argName] !== "") {
    // Clear the remaining values while keeping the existing other_categoryseller_name value
    filters["other_category" + argName] = "";
    filters = {
        other_category_category_name: filters.other_category_ticket,
        other_category_status: filters.other_category_status,
    };
  }
