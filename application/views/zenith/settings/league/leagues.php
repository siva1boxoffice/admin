   ...
   <div class="page-content">
      ...
      <div class="page-title-box">
         <div class="container-fluid">
            ...
            <div class="page-title dflex-between-center">
               <h3 class="mb-1">Top League Lists</h3>
               <div class="float-sm-right mt-3 mt-sm-0 add_team_s">
                  <a href="<?php echo base_url(); ?>settings/league/add"  class="btn btn-success mb-2">Add Top League</a>
               </div>
             </div>
         </div>
      </div>
         ...
         <div class="container-fluid">
            <div class="card">
               <div class="card-body">
                        ...
                        <div class="row">
                           ...
                           <div class="col-md-11">
							  ...
							  <li class="sort_list">
                                       <div class="btn-group">
                                          <div class="dropdown">
                                             <button class="btn btn-light dropdown-toggle league_search_filter" type="button"
                                                ...
                                                <div id="view_project_list_wrapper"
                                                   ...
                                                   <div id="view_project_list_filter" class="dataTables_filter"><label
                                                         class="search-box d-inline-flex position-relative">Search:<input
                                                            ...
                                                            id="league" placeholder="Search in Filters..."
                                                            aria-controls="view_project_list"></label></div>
                                                </div>
                                                <div class="reset_btn">
                                                   <div class="reset_txt"><button
                                                         class="btn btn-info league_reset">Reset</button></div>
                  ...
                  <div class="">
                     <table style='width:100% !important' id="top-league-list" class="table table-hover table-nowrap mb-0 tournament">
...
<script>
    ...
    var overlay = $('#overlay');
   var Dtable = $('#top-league-list').DataTable({
         'info': false,
         'serverSide': true,
         'serverMethod': 'post',
         'scrollX': !0,
         ...
         "bSort": false,
         "ajax": {
            url: base_url + 'settings/get_top_league_list',
            data: function (d) {
	  ...
	  $("body").on("click",".status_reset",function(){
		...
		if (checkedCount > 0) {
		 ...
		 $("body").on("click",".search_ok",function(){
			$('.league_search_filter').addClass("filter_active");
			applyFilters();
			//Dtable.ajax.reload();
       });

	   $("body").on("click",".league_reset",function(){
         $('.league_search_filter').removeClass("filter_active");
		 $('#league').val('');
          updateFilters("league");
            Dtable.ajax.reload();
         });
		 ...
		 function applyFilters() {
            ...
            sessionStorage.setItem('league_filter', JSON.stringify(filters));
          Dtable.draw();        
...
var storedFilters = sessionStorage.getItem('league_filter');
  ...
  var filters = JSON.parse(sessionStorage.getItem('league_filter'));
