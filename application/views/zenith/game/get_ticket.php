...
<?php if (@$sellTicketList)
                     ...
                     <form class="checkoutform" id="checkout-<?php echo $value['s_no'] ;?>" action="<?=base_url('game/orders/checkout'); ?>" method="post">
                                 ...
                                 <input type="hidden" name="tixstock_id" value="<?php echo $value['tixstock_id'] ;?>"   />
                                 <input type="hidden" value="<?php echo @$value['oneclicket_id'] ;  ?>" name="oneclicket_id">  

                                                 <input type="hidden" value="setsession" name="sessionset">
                              </form>

                               <div  id="seatid-<?php echo $value['s_no'] ;?>" data-ticketcategory="<?php echo $value['ticket_category'] ;?>" data-quantity="<?php echo $value['quantity'] ;?>" data-class="<?php echo $value['ticket_category'] ;?>" map-id="0" color-code="<?php echo $value['block_color'] ;?>" currency-price="<?php echo $value['price_with_symbol'] ;?> tick_sec" class="seat_select_block_items is-multiline is-flex-tablet-p seat_select_items field <?php echo $value['ticket_category'] ;?>" data-target="<?php echo $value['ticket_category'] ;?>" data-blockid="0" data-matchid="<?php echo $value['match_id'] ;?>" onmouseover="showMouseOver(this)" onmouseout="hideMouseOver(this)">
                                 <div class="tick_sec">
                                    <div class="row">
                                       <div class="col-lg-4">
                                          <div class="tick_all_items">
                                             <label class="tick_tier" for="example-select"><?php echo $value['seat_category'] ;?> <img src="<?php echo base_url();?>assets/zenith_assets/img/pdf.png"></label>
