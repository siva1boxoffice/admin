                                 ...
                                 <div class="card-body">
                                       ...
                                       <div id="TopAirLine_newww" class="topAirSlider owl-carousel owl-theme owl-loaded owl-drag">
                                                   ...
                                                   <?php for ($i = 1; $i <= $orderData->quantity; $i++) {
                                                      $tcktIdArray[]=$eticketData[$i - 1]->ticketid;                                                      
                                                      ?>
                                                      <div class="item">
                                                      ...
                                                      <?php }
                                                     // $tcktIdArray = json_encode($tckt_id);
                                                       ?>
                                                </div>
                                             </div>
...
<div class="button-list files_btn mt-2">
   ...
   <input class='etickets_more' name="eticket_more[]" type="file"  accept="image/jpeg,image/png,application/pdf"  style="display:none" multiple/>
     <button type="button" class="btn btn-success waves-effect waves-light add_more_files" data-effect="wave">Add More Files</button>
...
$(".add_more_files").click(function (e) {
   $('.etickets_more').trigger("click");
});

$("body").on("change",".etickets_more",function(){  
   //data_ticket_id
   //$tckt_id=
   var tckCnt= <?php echo count($tcktIdArray); ?>;
  var tcktIdArray = <?php echo json_encode($tcktIdArray); ?>;

      var selectedFiles = $(this).get(0).files;
      
      if (selectedFiles.length === 0) {
         swal('Updation Failed !', 'Please select one or more files.', 'error');
          return;
        }
     else if(selectedFiles.length>tckCnt)
      {
         swal('Updation Failed !', 'You can select only '+(tckCnt)+' Tickets.', 'error');
         return;
      }
      ...
      var formData = new FormData();
       ...
       // formData.append('data_ticket_id', tcktIdArray);
       var tcktIdArrayAsString = JSON.stringify(tcktIdArray);
      formData.append('data_ticket_id', tcktIdArrayAsString);

        $.ajax({
          url: '<?php echo base_url();?>game/multiple_upload_tickets',
          type: "POST",
          data: formData,
          processData: false,
          contentType: false,
         ...
         dataType: "json",
          success: function(data) {
            ...
            if(data.status == 1) {
               for (let i = 1; i <= data.show_file_name.length; i++) {
   // console.log(i);
             var temp="temp";
             //data.delete_ticket_id=1;
             id=1;
                 $('.show_preview_file_'+i).html('<embed type="application/pdf" class="d-block img-fluid embed_file" src="'+data.temp_file_name[i-1]+'"   style="overflow: hidden !important;" scrolling="no"/> <p class="show_file_name" id="show_file_name_'+i+'">'+data.show_file_name[i-1]+'</p> ');

                 $('.ticket_action_'+i).html('<ul> <li><div class="remove_ico"><i class=" far fa-trash-alt delete_ticket" data-delete-id=\''+data.delete_ticket_id[i-1]+'\'></i></div></li>  <li><div class="view_ico"><a target="_blank" href="javascript:void(0);" onclick="return popitup(\''+data.show_file_name[i-1]+'\',\''+temp+'\')" class="view_bg"><i class="far fa-eye"></i></a></div></li></ul>');    

                 $('.ticket_action_'+i).css("display", "block");
                 $('#drag_ticket_'+i).html(data.show_file_name[i-1]);
                 
                 ticket_arr.push({id: data.show_file_name[i-1]});
                ticket_arr.push(data.show_file_name[i-1]);
               }
         ...
         $.ajax({
                    ...
                    success: function (response) {
                     ...
                     // console.log('Success');
                      ...
                      //setTimeout(window.location.href= "<?php //echo base_url();?>game/orders/details/<?php //echo md5(//$orderData->booking_no);?>",200);
                    },
                    error: function () {
                       console.log('Failed');
                    }
...
//$(".delete_ticket").click(function () {   
   ...
   {
      if(temp!="")
            var url='<?php echo TICKET_UPLOAD_PATH."uploads/e_tickets/temp/";?>'+url; 
